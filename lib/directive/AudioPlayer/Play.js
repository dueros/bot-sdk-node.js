...
class Play extends BaseDirective {
    ...
    setOffsetInMilliSeconds (milliSeconds) {
        let offsetInMilliSeconds = parseInt(milliSeconds, 10);
        if (typeof offsetInMilliSeconds === 'number') {
            this.data.audioItem.stream.offsetInMilliSeconds = offsetInMilliSeconds;
        }
    }
    ...
    setPlayerInfo(playerInfo) {
        if (playerInfo instanceof PlayerInfo) {
            this.data.audioItem.playerInfo = playerInfo.getData();
        }
    }
}
