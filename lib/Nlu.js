...
class Nlu {
     ...
     */
    setIntentOption(value, entity, synonyms, index) {
        let option = {
            type: 'KEYWORD',
            value: '',
            entity: entity,
        ...
        if(synonyms instanceof Array && synonyms.length <= 5){
            option.synonyms = synonyms;
        }
        if(typeof index == 'number'){
            option.index = index;
        }

        option.updatedIntent = this._getUpdateIntent();

        this._intentOptions.push(option);
    }
     ...
     * @param {int} index 当前元素的位置，从1开始。选项中不同元素可以有相同的index。
     */
    setOption(value, entity, synonyms, index) {
        let option = {
            type: 'KEYWORD',
            value: '',
            entity: entity,
