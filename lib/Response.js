...
class Response {
    ...
    setExpectSpeech(expectSpeech) {
        if (typeof expectSpeech === 'boolean') {
            this._expectSpeech = expectSpeech;
        }
    }
    ...
    getTemplateToken() {
        let data = this._request.getData();
        let token = '';
        if (data && data.context && data.context.Screen && data.context.Screen.token) {
    ...
    buildResult(data) {
        ...
        directives = directives.map(function (directive) {
            if (directive instanceof BaseDirective) {
                ...
                if (data && data.type === 'DPL.ExecuteCommands') {
                    directive.setToken(self.getTemplateToken());
                }
        ...
        if (typeof this._expectSpeech === 'boolean') {
            ret.response.expectSpeech = this._expectSpeech;
        }
